...
#SBATCH --array=0-11 --time 48:00:00
#
...
with open('Coin_performance_%s.csv' % task_id, mode='w') as file:
    ...
    performance_table.writerow(['base_function', 'two_slope_fill_in_epsilon', 'perturbation_epsilon', '# breakpoints','# slopes','# vertices','# additive vertices','add_v/v ratio','delta pi min','is subadditive','.min(constant/BFS)','.min(lp<=50/BFS)','.min(lp<=500/BFS)','.min(lp<=10000/BFS)','.min(constant/DFS)','.min(lp<=50/DFS)','.min(lp<=500/DFS)','.min(lp<=10000/DFS)','.min(constant/BB)','.min(lp<=50/BB)','.min(lp<=500/B)','.min(lp<=10000/BB)','naive subadditivity test (s)'])
    for two_slope_fill_in_epsilon in [0,QQ(1)/10,QQ(1)/20,QQ(1)/50,QQ(1)/100, QQ(1)/1000]:
        ...
        if not previous_f:
            ...
            if new_f==previous_f:
                continue
            previous_f=new_f
        for perturbation_epsilon in [0,QQ(1)/1000,QQ(1)/100,QQ(1)/10]:
            global fn
            ...
            res_0_BB=measure_T_min(fn,0,'BB',solver='Coin',number=1,repeat=1)
            res_0_BB.append(float(res_0_BB[1]/v))
            res_50_BB=measure_T_min(fn,50,'BB',solver='Coin',number=1,repeat=1)
            res_50_BB.append(float(res_50_BB[1]/v))
            res_500_BB=measure_T_min(fn,500,'BB',solver='Coin',number=1,repeat=1)
            res_500_BB.append(float(res_500_BB[1]/v))
            res_10000_BB=measure_T_min(fn,10000,'BB',solver='Coin',number=1,repeat=1)
            res_10000_BB.append(float(res_10000_BB[1]/v))
            naive_time=sage_timeit('minimum_of_delta_pi(fn)',globals(),seconds=True,number=1,repeat=1)
            performance_table.writerow([factory,two_slope_fill_in_epsilon,perturbation_epsilon,len(fn.end_points()),number_of_slopes(fn),v,add_v,float(add_v/v),delta_pi_min,is_sub,res_0_B,res_50_B,res_500_B,res_10000_B,res_0_D,res_50_D,res_500_D,res_10000_D,res_0_BB,res_50_BB,res_500_BB,res_10000_BB,naive_time])
