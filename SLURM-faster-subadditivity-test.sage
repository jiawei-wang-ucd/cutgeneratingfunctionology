...
with open('cplex_performance_%s.csv' % task_id, mode='w') as file:
    ...
    for two_slope_fill_in_epsilon in [0,QQ(1)/10,QQ(1)/20,QQ(1)/50,QQ(1)/100, QQ(1)/1000]:
        ...
        for perturbation_epsilon in [0,QQ(1)/1000,QQ(1)/500,QQ(1)/100,QQ(1)/50]:
            ...
            res_100_D=measure_T_min(fn,100,'DFS',solver='cplex',number=1,repeat=1)
            res_100_D.append(float(res_100_D[1]/v))
            res_500_D=measure_T_min(fn,500,'DFS',solver='cplex',number=1,repeat=1)
            res_500_D.append(float(res_500_D[1]/v))
            res_10000_D=measure_T_min(fn,10000,'DFS',solver='cplex',number=1,repeat=1)
            res_10000_D.append(float(res_10000_D[1]/v))
            res_0_B=measure_T_min(fn,0,'BFS',solver='cplex',number=1,repeat=1)
            res_0_B.append(float(res_0_B[1]/v))
            res_20_B=measure_T_min(fn,20,'BFS',solver='cplex',number=1,repeat=1)
            res_20_B.append(float(res_20_B[1]/v))
            res_50_B=measure_T_min(fn,50,'BFS',solver='cplex',number=1,repeat=1)
            res_50_B.append(float(res_50_B[1]/v))
            res_100_B=measure_T_min(fn,100,'BFS',solver='cplex',number=1,repeat=1)
            res_100_B.append(float(res_100_B[1]/v))
            res_500_B=measure_T_min(fn,500,'BFS',solver='cplex',number=1,repeat=1)
            res_500_B.append(float(res_500_B[1]/v))
            res_10000_B=measure_T_min(fn,10000,'BFS',solver='cplex',number=1,repeat=1)
